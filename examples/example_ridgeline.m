...
p.Nx = 100;                 %     p.Nx             Number of grid points in x direction
p.Ny = 100;                 %     p.Ny             Number of grid points in y direction
p.dx = 4;                   %     p.dx             Grid spacing in the x direction (m)
p.dy = 4;                   %     p.dy             Grid spacing in the y direction (m)
...
p.dtmax = 1000;             %     p.dtmax          maximum time step (yr)
p.Courant = 0.9;            %     p.Courant        maximum Courant number
p.tf = 2e6;                 %     p.tf             Total time of the simulation (yr)
...
p.E = 1e-4;                 %     p.E              Rate of surface uplift or base level lowering (m/yr)
...
p.flood = 0;                %     p.flood          1=route flow through local minima, 0=don't
...
p.plotint = 50;             %     p.plotint        Plot will be redrawn every plotint iterations
p.plottype = 'perspective'; %     p.plottype       Options are 'perspective', 'mesh', 'drainage area', 'curvature', 'elevation', 'contour', 'shade', 'color shade'
...
p.runname = 'example_ridge';%     p.runname:       Character string naming the run. If specified 
...
p.doDiffusion = 1;          %     p.doDiffusion    Turn hillslope diffusion on (1) or off (0)
p.D = 0.01;                 %     p.D              Hillslope diffusivity (m^2/yr)
                            %
...
p.Kf = 1e-4;                %     p.Kf             Coefficient in stream power incision law (kg m^(1+2m) yr^-2)
...
p.thetac = 5;               %     p.thetac         Threshold for fluvial incision



% ------------------ initial conditions -----------------------------------                           

p.beta = 1;                 %     p.beta           Negative slope of the power spectrum. 0 = white noise, more positive values are "redder" (more variance at longer wavelengths)
