...
p.runname = 'circle';       %     p.runname:       Character string naming the run. If specified 
...
xc = (p.Nx-1)/2+1; % center of circle
yc = (p.Ny-1)/2+1;
r = (p.Nx-1)/2; % radius of circle
S = 0.05; % slope of cone
[X Y] = meshgrid(1:p.Nx,1:p.Ny);
D = sqrt((X-xc).^2 + (Y-yc).^2);
p.F(D >= r) = 1; % hold elevations of "offshore" points fixed
cone = (r - D)*p.dx*S; % the initial cone profile
cone(p.F==1) = 0; % set elevations of offshore points to sea level


noise = RedNoise(p.Ny,p.Nx,p.beta,p.variance);

noise = noise - min(noise(:));
noise(p.F==1)=0;

% taper noise to zero at edge of shield
noise = noise .* cone/max(cone(:));
